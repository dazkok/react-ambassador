...
const Header = (props: {user: User}) => {
    ...
    const [description, setDescription] = useState('Share this link to earn money');

    useEffect(() => {
        if (props.user?.id) {
            setTitle(`${props.user.revenue.toFixed(2)} PLN`)
            setDescription('You have earned this far');
        } else {
            setTitle('Welcome')
            setDescription('Share this link to earn money');
        }
    }, [props.user]);
